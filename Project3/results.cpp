...
int main() {
  ...
  PenningTrap obj1 = PenningTrap(B0, V0, d);
  PenningTrap obj2 = PenningTrap(B0, V0, d);
  PenningTrap obj3 = PenningTrap(B0, V0, d);
  PenningTrap obj4 = PenningTrap(B0, V0, d);
  PenningTrap obj5 = PenningTrap(B0, V0, d);
  PenningTrap obj6 = PenningTrap(B0, V0, d);
  PenningTrap obj7 = PenningTrap(B0, V0, d);
  PenningTrap obj8 = PenningTrap(B0, V0, d);
  PenningTrap obj9 = PenningTrap(B0, V0, d);
 PenningTrap obj10 = PenningTrap(B0, V0, d);
 PenningTrap obj11 = PenningTrap(B0, V0, d);
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < n; j++) {
      PenningTrap obj = PenningTrap(B0, V0, d, f(i), w(j));
      obj.add_random_particles(100, 1,  m);
  ...
  int n1 = 20;
  arma::vec w1 = arma::linspace(2.05, 2.35, n1);
  arma::vec f1 = arma::vec("0.1 0.4 0.7");
  arma::mat particles_left1 = arma::mat(3, n1);
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < n1; j++) {
      PenningTrap obj = PenningTrap(B0, V0, d, f1(i), w1(j));
      obj.add_random_particles(100, q,  m);
      obj.simulation(500. / 5000, 5000, "RK4");
  ...
  int n2 = 20;
  arma::vec w2 = arma::linspace(2.05, 2.35, n2);
  arma::vec f2 = arma::vec("0.1 0.4 0.7");
  arma::mat particles_left2 = arma::mat(3, n2);
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < n2; j++) {
      PenningTrap obj = PenningTrap(B0, V0, d, f2(i), w2(j));
